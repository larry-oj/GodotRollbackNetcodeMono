...
func _call_get_local_input() -> Dictionary:
	...
	for node in nodes:
		if _network_adaptor.is_network_master_for_node(node) and node.is_inside_tree() and not node.is_queued_for_deletion():
			var node_input = {}
			if node.has_method('_get_local_input'):
				node_input = node._get_local_input()
			elif node.has_method('_GetLocalInput'):
				node_input = node._GetLocalInput()
...
func _call_network_process(input_frame: InputBufferFrame) -> void:
	...
	while i > 0:
		...
		if node.is_inside_tree() and not node.is_queued_for_deletion():
			...
			if node.has_method('_network_process') or node.has_method('_NetworkProcess'):
				process_nodes.append(node)
			if node.has_method('_network_postprocess') or node.has_method('_NetworkPostprocess'):
	...
	for node in process_nodes:
		if node.is_inside_tree() and not node.is_queued_for_deletion():
			...
			if (node.has_method('_network_process')):
				...
				node._NetworkProcess(player_input.get(str(node.get_path()), {}))
	...
	for node in postprocess_nodes:
		if node.is_inside_tree() and not node.is_queued_for_deletion():
			...
			if node.has_method('_network_postprocess'):
				...
				node._NetworkPostprocess(player_input.get(str(node.get_path()), {}))

func _call_save_state() -> Dictionary:
	var state := {}
	var nodes: Array = get_tree().get_nodes_in_group('network_sync')
	for node in nodes:
		if (node.has_method('_save_state') or node.has_method("_SaveState")) and node.is_inside_tree() and not node.is_queued_for_deletion():
			var node_path = str(node.get_path())
			if node_path != "":
				if node.has_method('_save_state'):
					state[node_path] = node._save_state()
				else:
					state[node_path] = node._SaveState()
...
func _call_load_state(state: Dictionary) -> void:
	for node_path in state:
		...
		elif node.has_method('_LoadState'):
			node._LoadState(state[node_path])
...
func _call_interpolate_state(weight: float) -> void:
	for node_path in _interpolation_state:
		...
		if node:
			...
			elif node.has_method('_InterpolateState'):
				node._InterpolateState(states[0], states[1], weight)
...
func _predict_missing_input(input_frame: InputBufferFrame, previous_frame: InputBufferFrame) -> InputBufferFrame:
	if not input_frame.is_complete(_player_peers):
		...
		for node in nodes:
			...
			var has_PredictNetworkInput: bool = node.has_method('_PredictNetworkInput')
			if (has_predict_network_input or has_PredictNetworkInput) or previous_input.has(node_path_str):
				...
				var predicted_input_for_node = {}
				if has_predict_network_input:
					node._predict_remote_input(previous_input_for_node, ticks_since_real_input) if has_predict_network_input else previous_input_for_node.duplicate()
				elif has_PredictNetworkInput:
					node._PredictRemoteInput(previous_input_for_node, ticks_since_real_input) if has_PredictNetworkInput else previous_input_for_node.duplicate()
...
func _physics_process(_delta: float) -> void:
	...
	if debug_physics_process_msecs > 0 and total_time_msecs > debug_physics_process_msecs:
		push_warning("[%s] SyncManager._physics_process() took %.02fms" % [_current_tick, total_time_msecs])
